...
static enum type check_expr_internal(ast_node *node, symbol_table *local_tbl, int is_negative);
...
static enum type check_expr(ast_node *node, symbol_table *local_tbl) {
    return check_expr_internal(node, local_tbl, 0);
}
...
static enum type check_expr_internal(ast_node *node, symbol_table *local_tbl, int is_negative) {
    ...
    switch (node->category) {
        case Natural: {
            ...
            unsigned long long max_val = is_negative ? 2147483648ULL : 2147483647ULL;
            if (v > max_val) {
        ...
        case ParseArgs: {
            ...
            if (arr_t != type_string_array || idx_t != type_int) {
        ...
        case Assign: {
            ...
            int valid = 0;
            if (ltype == rtype && ltype != type_undef && ltype != type_string_array) valid = 1;
            else if (ltype == type_double && rtype == type_int) valid = 1;
        ...
        case Minus: {
            int child_is_neg = 0;
            if (node->child && node->child->category == Natural) {
                child_is_neg = 1;
            }
            enum type ot = check_expr_internal(node->child, local_tbl, child_is_neg);
            
            // remover proteção contra type_undef
            if (ot != type_int && ot != type_double) {
